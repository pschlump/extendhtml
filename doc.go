...
/*
Package template (github.com/pschlump/extendhtml) implements data-driven templates for
...
but the contextual autoescaping in github.com/pschlump/extendhtml

	import  (
		template "github.com/pschlump/extendhtml"
	)
	...
	t, err := template.New("foo").Parse(`{{define "T"}}Hello, {{.}}!{{end}}`)
	err = t.ExecuteTemplate(out, "T", "<script>alert('you have been pwned')</script>")
